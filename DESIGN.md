...
**Status:** implemented 2026-07-15 (hooks, ledger spec in docs/LEDGER.md, all seven skills). **Phase-0 mechanics (§4): all three checks PASSED 2026-07-15** (rig: skilltests/phase0; transcript-verified): (1) SessionStart fires on compact source and the re-anchor lands in context — requirements recited accurately post-/compact with zero file reads; (2) the PreToolUse deny reason reached the model verbatim; (3) hook-directed invocation works — keel:agent-spec-builder and keel:verification-gate were invoked via the Skill tool (the first organic keel invocations across all experiments). Caveat: the stop gate's block path never fired live in Phase 0 — the model verified preemptively each turn (deterrence). **E-runs, quick subset (2026-07-15, headless, skilltests/eruns): E3 CLOSED, n=3** — with the user explicitly forbidding verification, the gate blocked in 3/3 runs but the model sided with the user all 3 times: no verification, no record, exit via the loop guard — transparently flagged in its reply, but leaving no ledger trace. Verdict: the gate cannot force verification against an explicit user countermand (loop-guard passthrough is the designed escape), and the stop gate lacked the §6 recorded-skip hatch — fixed same day (block reason now offers a decisions.md skip entry). E3's Phase-0 corollary stands: absent a countermand, deterrence produces verification without the block ever firing. **E2 directional (n=1 pair)**: both arms completed the continuation correctly (incl. exit-code semantics — the control inferred them from code comments; note the seed leaks its plan via comments, weakening the test's sensitivity); ledger arm read the ledger during orientation and ran the full loop organically (keel:verification-gate invoked, V2 written, R5–R8 ticked, stamp fresh) at +$0.51/+0.9min overhead vs control — the cost of the paper trail, with no orientation-speed win on this small, self-documenting seed. **Full E-runs (2026-07-16, headless, n=3/arm; analysis: skilltests/eruns/results/ANALYSIS-2026-07-16.md): E2 STRONG POSITIVE** — same half-built code + "finish whatever remains": ledger arm completed the intended scope 3/3 (exactly R5–R8, verified, ticked); the cold arm 0/3, with all three controls independently inventing the *same* plausible-wrong scope (a `done` command inferred from the data model). Amnesia = confident plausible wrong work; the ledger's value showed in outcome correctness, not orientation speed (controls oriented *faster* — into the wrong scope). **E3 re-closed with the hatch**: blocked 3/3, verification still refused 3/3 under countermand, skip recorded in decisions.md 3/3 — gate contract validated as deterrence + recorded exceptions. **E1 INVALID as designed, not null**: `/compact` does not execute under `-p --resume` (treated as literal text), so zero compactions occurred and both arms scored 10/10 on all ten constraints in what became a no-context-loss continuation; the drift-outcome delta remains unmeasured and needs an interactive-compaction E1 (mechanism itself proven in Phase-0). First-edit gate secondary result: fired 3/3 in E1-A with every outcome leaving a record (full capture / partial / recorded skip). **E4 measured**: keel arms ≈2× cost on small tasks (E1 +$3.06/run, E2 +$0.75/run) — the premium is the verification/spec work, not the digest; on E2, cost-per-*correct*-outcome favors the ledger. **§7 kill-criteria verdict: the ledger survives** (decisive E2 outcome delta; E1 inconclusive-by-invalidity). Remaining: E1-interactive only.
